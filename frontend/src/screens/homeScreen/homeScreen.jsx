...
function HomeScreen() {
    ...
    const handleProfAPICall = async (name) => {
        setIsLoading(true);
        try {
            const response = await fetch('https://graderu.herokuapp.com/api/v1/professors?name=' + name, {
                method: 'GET',
                headers: {
    ...
    const handleCourseAPICall = async (input) => {
        ...
        try {
            ...
            const response = await fetch('https://graderu.herokuapp.com/api/v1/courses?subject=' + words[0] + '&number=' + words[1], {
                method: 'GET',
                headers: {
                    Accept: 'application/json',
                },
